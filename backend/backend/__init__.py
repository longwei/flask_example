...
def create_app():
    ...
    app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://longwei:my-password@localhost:5432/backenddb"
...
def feed_db():
    ...
    user = db.session.scalars(select(User).where(User.username == "longwei")).one()
